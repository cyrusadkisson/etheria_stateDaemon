		...
		dynamoDB.query(params, function(err, oldData) {
			if (err) {
				...
				if (oldData.Items.length === 0) {
					...
					var keyToGet = "NONEXISTENTKEY";
					if (typeof oldData.Items[0].state === "string" && oldData.Items[0].state.startsWith("state-")) {
						keyToGet = oldData.Items[0].state;
					}

					var params = {
						Bucket: "etheria-states",
						Key: keyToGet // we know this is a key or NONEXISTENTKEY
					};

					s3.getObject(params, function(s3err, s3data) {
						if (s3err) {
							// no big deal. we looked for NONEXISTENTKEY, proceed with oldData.Items[0].state as-is (a blob of binary data, direct from dynamodb)
							console.log("s3err block looking for NONEXISTENTKEY. no problem, just keep going with oldData as-is");
						} else {
							console.log("s3 object retrieved with length " + s3data.Body.length); // Convert buffer to string
							oldData.Items[0].state = s3data.Body; // replace the pointer key with the actual binary data
						}
								...
								else	// we found a block with a relevant transaction. Do a whole state pull at numberOfFirstRelevantBlock
								{
									...
									lambda.invoke({
										...
										if (newMapEnvelope.tiles) { // a check to make sure what we're getting back has a valid map
											...
											var compressed = pako.deflate(JSON.stringify(newMapEnvelope.tiles), { to: 'string' }); // FIXME This should come below, only if the states are different, right?
											...
											if (eventStrings === true) // nothing changed. state is the same. Update existing row and keep searching
											{
												...
												var s3params = {
													Bucket: "etheria-states",
													Key: "state-" + numberOfFirstRelevantBlock + "-" + event.params.querystring.version.replaceAll(".", "pt"),
													Body: compressed,
													...
													if (err) {
														...
														var params = {
															...
															Item: {
																...
																'state': "state-" + numberOfFirstRelevantBlock + "-" + event.params.querystring.version.replaceAll(".", "pt"),
